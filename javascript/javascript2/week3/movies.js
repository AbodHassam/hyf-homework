...
console.log('%cRating TAG','font-weight:bold;color:red');
console.log(moviewTag);


//maping the movies array to only the rating of the movies.
const moviesRating = moview.map(movie => movie.rating);
console.log('%cMovies rating','font-weight:bold;color:red');
console.log(moviesRating);


//filtering the movies array to only contain the movies rated higher than 6.
const highterThan6 = moviesRating.filter(rating => rating >6);
console.log('%cMovies rated higher than 6','font-weight:bold;color:red');
console.log(highterThan6);


//Counting the number of movies containing the following keywords: ["The", "dog", "who", "is", "not", "a", "man"]. 
let counter = 0;
let keywords= ["The", "dog", "who", "is", "not", "a", "man"];
for (let i = 0; i < keywords.length; i++) {
    //const specificMovies = moview.filter(movie => movie.title.includes(keywords[i])).
    moview.map(movie => {
...
console.log('%cMovies containing specific keywords','font-weight:bold;color:red');
console.log(counter);


//Calculating the average rating of all the movies using reduce.
const ratingSum=moview.reduce(totRating,0);
function totRating(accumulator,movie){
    accumulator=accumulator+movie.rating;
    return accumulator;
}
const averageRating=ratingSum/moview.length; 
console.log('%cAverage rating of all the movies','font-weight:bold;color:red');
console.log(averageRating.toFixed(1));

/* 
//Calculating the average rating of all the movies using reduce.
const averageRating = moviesRating.filter(rating => rating >= 4 && rating < 7);
  //console.log(averageRating);
const RatingCount = (total , val)=> total + val ;
console.log('%csum of movies rated as average','font-weight:bold;color:red');
console.log(averageRating.reduce(RatingCount).toFixed(1));
 */


//Calculating the total number of Good, Average and Bad movies using reduce.
let totalNumberOfGood = moview.filter(movie => movie.tag == 'Good' );
console.log('%cThe total number of Good','font-weight:bold;color:red');
console.log(totalNumberOfGood.length);

let totalNumberOfAverage = moview.filter(movie => movie.tag == 'Average' );
console.log('%cThe total number of Average','font-weight:bold;color:red');
console.log(totalNumberOfAverage.length);

let totalNumberOfBad = moview.filter(movie => movie.tag == 'Bad' );
console.log('%cThe total number of Bad','font-weight:bold;color:red');
console.log(totalNumberOfBad.length);
